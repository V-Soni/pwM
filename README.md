...
- **Contextual Chat**: The chat interface uses the MCQ results as an underlying system prompt context, allowing "Maa" to immediately reference their current state (e.g., "Beta, you said you didn't sleep at all. How will your brain work?").
...
2. **Backend**: FastAPI handles asynchronous API routing (`/api/mcq-submit` and `/api/chat`).
